...
class Block {
public:
	Block(unsigned index, Hashtype previoushash, Typtime timestamp, Datatype data, Hashtype hash, size_t nounce):
		index_(index),
...
public:
	...
	BlockChain( Block<Datatype>* FirstBlock ) {	AppendBlock( FirstBlock ); }

	Block<Datatype>* GenerateBlock( Block<Datatype>* PreviousBlock, Datatype new_data ) {
		...
		auto* new_Block = new Block<Datatype>( new_index, previous_hash, time_now, new_data, new_hash );
		return new_Block;
	}

	void MiningNewBlock( const Datatype& data, Miner<Datatype>* worker ) {
		size_t 		newnounce = 0;
		Hashtype 	newhash;
		auto* 		LastBlock = chain.back();
		worker->Mine( LastBlock->get_index(), data, LastBlock->get_hash(), newhash, newnounce );
		auto* 		MinedBlock = new Block<Datatype>(
	...
	void ValidateNewBlock( Block<Datatype>* NewBlock ) {
		if( chain.size() > 0 ) {
			auto* LastBlock = chain.back();
	...
	void AppendBlock( Block<Datatype>* new_block ) {
		ValidateNewBlock( new_block );
		chain.push_back( new_block );
	}

	void AppendChain( BlockChain<Datatype>* new_chain ) {
...
private:
	std::vector< Block<Datatype>* > chain;
	std::string name;
};
